...
@section('content')
        ...
        @forelse ($playlist as $item)
            <div class="col-sm-6 col-lg-6 mb-3">
                <div class="position-relative overflow-hidden youtube-video-item" style="height: 200px;">
                    <img class="img-fluid w-100 h-100" src="{{ asset($item->thumbnail) }}" style="object-fit: cover;">
                    <div class="overlay p-0">
                        <div
                            ...
                            <a class="" href="{{ route('youtube.playlist.view', $item->id) }}">
                        ...
                        <div class="mb-1 text-truncate"
                            ...
                            <span class="text-white">{{ $item->itemCount }} Videos</span>
                            <span class="px-1 text-white">/</span>
                            {{ \Carbon\Carbon::parse($item->published_at)->format('F d, Y') }}
                        </div>
                        <a class="h4 m-0 p-3 video-title " href="{{ route('youtube.playlist.view', $item->id) }}"
                            data-toggle="tooltip" data-title="{{ $item->title }}">{{ Str::limit($item->title, 75) }}</a>
                    </div>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-12">
            {{ $playlist->links('pagination::default') }}
        </div>
    </div>
@endsection
@section('styles')
    ...
    <style>
        ...
        .youtube-video-item .video-title {
            color: var(--color-white);
            text-shadow: 0px 0px 25px var(--color-primary);
            font-weight: bold;
            display: block;
            text-decoration: none;
        }
